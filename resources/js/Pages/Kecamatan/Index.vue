        ...
        <div class="space-y-8">
            ...
            >
                <div class="text-center">
                    <h2 class="text-3xl font-bold text-gray-900 mb-2">Data Kecamatan</h2>
            ...
            >
                <div class="px-8 py-6 border-b border-gray-200">
                    <h3 class="text-xl font-semibold text-gray-900">Tabel Data Kecamatan</h3>
                    <p class="text-gray-600 mt-1">Daftar kecamatan di {{ kabupatenName }}</p>
                </div>
                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium">
                                        <a :href="`/dpr/kecamatan/${kec.id}/kelurahan`"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        {{ formatNumber(kec.jumlah_kelurahan) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                        {{ formatNumber(kec.jumlah_tps) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                        {{ formatNumber(kec.total_dpt) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a :href="`/dpr/kecamatan/${kec.id}/tps`"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a :href="`/dpr/kecamatan/${kec.id}/export-tps-excel`"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a :href="`/dpr/kecamatan/${kec.id}/suara`"
                ...
                <div class="bg-gray-50 px-8 py-4 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-700">
                            Menampilkan <span class="font-medium">{{ kecamatan.length }}</span> kecamatan dari total
                            <span class="font-medium">{{ kecamatan.length }}</span> data
                        </p>
...
const statistics = computed(() => {
    const totalKelurahan = props.kecamatan.reduce((sum, k) => sum + parseInt(k.jumlah_kelurahan), 0)
    const totalTPS = props.kecamatan.reduce((sum, k) => sum + parseInt(k.jumlah_tps), 0)
    const totalDPT = props.kecamatan.reduce((sum, k) => sum + parseInt(k.total_dpt || 0), 0)

    return [
        {
            label: 'Total Kecamatan',
            value: props.kecamatan.length,
            icon: MapIcon,
