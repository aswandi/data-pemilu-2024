        ...
        <div class="space-y-8">
            ...
            >
                <div class="text-center">
                    <h2 class="text-3xl font-bold text-gray-900 mb-2">Data Kabupaten/Kota</h2>
                    <p class="text-xl text-gray-600 mb-4">{{ provinceName }}</p>
            ...
            >
                <div class="px-8 py-6 border-b border-gray-200">
                    <h3 class="text-xl font-semibold text-gray-900">Tabel Data Kabupaten/Kota</h3>
                    <p class="text-gray-600 mt-1">Daftar kabupaten/kota di {{ provinceName }}</p>
                </div>
                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium">
                                        <a :href="`/dpr/kabupaten/${kab.id}/kecamatan`"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                        {{ formatNumber(kab.jumlah_kecamatan) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        {{ formatNumber(kab.jumlah_kelurahan) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                        {{ formatNumber(kab.jumlah_tps) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                        {{ formatNumber(kab.jumlah_dpt) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a :href="`/dpr/kabupaten/${kab.id}/suara`"
                                       class="inline-flex items-center px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors duration-200 text-xs">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a :href="`/dpr/kabupaten/${kab.id}/suara/caleg/export-excel`"
                                       class="inline-flex items-center px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white font-medium rounded-lg transition-colors duration-200 text-xs">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gray-50 px-8 py-4 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-700">
                            Menampilkan <span class="font-medium">{{ kabupaten.length }}</span> kabupaten/kota dari total
                            <span class="font-medium">{{ kabupaten.length }}</span> data
                        </p>
...
const statistics = computed(() => {
    const totalKecamatan = props.kabupaten.reduce((sum, k) => sum + parseInt(k.jumlah_kecamatan || 0), 0)
    const totalKelurahan = props.kabupaten.reduce((sum, k) => sum + parseInt(k.jumlah_kelurahan || 0), 0)
    const totalTPS = props.kabupaten.reduce((sum, k) => sum + parseInt(k.jumlah_tps || 0), 0)
    const totalDPT = props.kabupaten.reduce((sum, k) => sum + parseInt(k.jumlah_dpt || 0), 0)

    return [
        {
            label: 'Total Kab/Kota',
            value: props.kabupaten.length,
            icon: MapIcon,
